...
**아이고 청년**은 사용자가 임대차 계약 관련 질의 시, **법령·판례·법령해석례를 검색하여 근거 기반으로 답변**하는 RAG기반 웹 서비스입니다.

본 4차 단위 프로젝트에서는 3차 단위에서 구현한 MVP를 **Django + FastAPI 분리 아키텍처 기반의 웹 서비스로 확장**하였으며, **회원 시스템, PDF 계약서 업로드 + OCR 텍스트 추출, 개인정보(PII) 자동 마스킹** 등 실서비스 운영에 필요한 기능을 추가 구현하였습니다.
...
본 서비스는 **임대차 게약서 내용과 특약**을 법령·판례 근거로 질의응답할 수 있는 **B2C 일반인 대상 대화형 AI 서비스**가 목표입니다.

1. **"주소"가 아닌 "내용"에 집중**
   - 공공 서비스가 _'이 집이 안전한가?'_ 를 묻는다면, 우리는 _'내가 사인할 이 계약서의 문구가 나에게 안전한가?'_ 에 답합니다.

2. **임대차 특화 RAG 파이프라인**
   - 일반 법률 데이터가 아닌 **주택임대차보호법, 실제 판례, 법령 해석례**에 한정한 도메인 특화 검색을 통해 생성형 AI의 할루시네이션을 최소화하고 정확한 법적 근거를 제시합니다.
...
- 무료 기본 서비스 + **프리미엄 구독**
- 법률 자문이 필요한 사용자에게 **외부 전문가 플랫폼 연계 링크** 제공
...
| **배포 환경**        | 로컬 실행 (Streamlit run)   | **AWS EC2 + Docker + Nginx**       |
...
4. **답변 출처 링크 자동 삽입** — 답변 생성 시 참조한 원문을 출처와 함께 제공 
...
| 진세형 | PDF 텍스트 추출(PyMuPDF를 통한 추출, OCR 추출)을 위한 langgraph 구축 및 계약서 업로드를 위한 Django 앱을 생성하고 관련 프런트 구현을 수행했습니다.<br/>PDF 업로드 과정에서 발생한 어려움을 완전히 해소하지 못해 무결한 시스템을 구축하는 데에 어려움이 발생했습니다. 서비스 배포를 위한 코드를 작성할 때 로컬 환경만 고려하지 말고 배포했을 떄의 코드 적합성을 고려하겠습니다. 서비스 배포 기술 숙달과 분업 전략도 잘 수립하여, 빠르고 통일성 있게 업무를 수행할 수 있도록 하겠습니다.|
