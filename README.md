...
| 진세형 | PDF 텍스트 추출(PyMuPDF를 통한 추출, OCR 추출)을 위한 langgraph 구축 및 계약서 업로드를 위한 Django 앱을 생성하고 관련 프런트 구현을 수행했습니다. PDF 업로드 과정에서 발생한 어려움을 완전히 해소하지 못해 무결한 시스템을 구축하는 데에 어려움이 발생했습니다. 서비스 배포를 위한 코드를 작성할 때 로컬 환경만 고려하지 말고 배포했을 떄의 코드 적합성을 고려하겠습니다. 서비스 배포 기술 숙달과 분업 전략도 잘 수립하여, 빠르고 통일성 있게 업무를 수행할 수 있도록 하겠습니다.|
