...
def test_email_send_signup_success(client):
  res = client.post(
    f"{BASE}/email/send/", {"email": "new@example.com", "purpose": "SIGNUP"}, format="json"
  )
...
def test_email_send_signup_rejects_existing_email(client, user):
  res = client.post(
    f"{BASE}/email/send/", {"email": user.email, "purpose": "SIGNUP"}, format="json"
  )
...
def test_email_send_reset_rejects_unknown_email(client):
  res = client.post(
    f"{BASE}/email/send/", {"email": "ghost@example.com", "purpose": "RESET"}, format="json"
  )
  assert res.status_code == 400
...
def test_email_verify_wrong_code(client):
  ...
  res = client.post(
    f"{BASE}/email/verify/",
    {"email": "new@example.com", "code": "000000", "purpose": "SIGNUP"},
    format="json",
  )
  assert res.status_code == 400
...
def test_login_wrong_password(client, user):
  res = client.post(
    f"{BASE}/login/", {"email": user.email, "password": "WrongPass1!"}, format="json"
  )
