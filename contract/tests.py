...
class ContractTest(TestCase):
  def setUp(self):
    ...
    self.user = User.objects.create_user(
      email="test@test.com", password="testpass123", nickname="테스터"
    )
  ...
  @patch("aigo_ai.client.httpx.post")
  def test_TC13_upload_pdf_success(self, mock_post):
    ...
    response = self.client.post(
      f"/contract/{self.chatroom_id}/upload/", {"file": pdf_file}, format="multipart"
    )
  ...
  def test_TC25_upload_not_pdf(self):
    ...
    response = self.client.post(
      f"/contract/{self.chatroom_id}/upload/", {"file": jpg_file}, format="multipart"
    )
  ...
  def test_TC26_upload_over_5mb(self):
    ...
    response = self.client.post(
      f"/contract/{self.chatroom_id}/upload/", {"file": big_file}, format="multipart"
    )
    self.assertEqual(response.status_code, 400)
    data = json.loads(response.content)
    self.assertFalse(data["success"])
  ...
  def test_TC28_update_property_partial(self):
    ...
    # location만 수정
    response = self.client.post(
      f"/contract/{self.chatroom_id}/property/", {"location": "서울시 마포구"}
    )
    self.assertEqual(response.status_code, 200)
  ...
  def test_unauthorized_upload(self):
    ...
    other_user = User.objects.create_user(
      email="other@test.com", password="otherpass123", nickname="다른유저"
    )
