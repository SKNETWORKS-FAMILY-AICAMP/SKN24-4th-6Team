...
from .serializers import ProfileUpdateSerializer, SelfVerifySerializer
...
class SelfVerifyView(APIView):
  ...
  def post(self, request):
    ...
    if not request.user.check_password(serializer.validated_data["password"]):
      return Response(
        {"error": "비밀번호가 일치하지 않습니다."}, status=status.HTTP_401_UNAUTHORIZED
      )

    request.session["self_verified"] = True

    return Response({"message": "본인인증이 완료되었습니다."}, status=status.HTTP_200_OK)
...
class MeView(APIView):
  ...
  def _check_self_verified(self, request):
    if not request.session.get("self_verified"):
      return Response({"error": "본인인증이 필요합니다."}, status=status.HTTP_403_FORBIDDEN)
    return None
